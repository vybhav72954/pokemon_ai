...
def _main_option_key(obs: Observation, opt: Option) -> tuple:
...
def _choose_main(obs: Observation, select: SelectData) -> list[int]:
    best_i = min(range(len(select.option)), key=lambda i: _main_option_key(obs, select.option[i]))
    return [best_i]
...
# ---- SelectType.EVOLVE / ATTACHED_CARD / CARD_OR_ATTACHED_CARD --------------
# These select a fully-specified (source card, target Pokemon) pair. Prefer
...
def _decide(obs: Observation) -> list[int]:
    ...
    if select.type in (SelectType.EVOLVE, SelectType.ATTACHED_CARD, SelectType.CARD_OR_ATTACHED_CARD):
